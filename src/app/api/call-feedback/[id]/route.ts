...
export async function PATCH(
  ...
  try {
    ...
    const current = await prisma.callFeedback.findUnique({ where: { id } });
    if (!current) {
      return NextResponse.json({ error: "Kayıt bulunamadı" }, { status: 404 });
    }
    ...
    const row = await prisma.callFeedback.update({
      where: { id },
      data: {
    ...
    logActivity(
      auth.user!,
      "CAGRI_GERIBILDIRIM_GUNCELLE",
      `Çağrı geribildirimini güncelledi: ${row.personelName} — ${row.topic}.`,
      {
...
export async function DELETE() {
  return NextResponse.json(
    { error: "Çağrı geribildirim kayıtları silinemez. Geçmiş korunur." },
    { status: 405 },
  );
}
